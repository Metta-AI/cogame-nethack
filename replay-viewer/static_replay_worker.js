...
function stageNote() {
  ...
  try {
    var length = Module._nethack_stage_len ? Module._nethack_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._nethack_stage_ptr();
...
function runtimeError() {
  var length = Module._nethack_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._nethack_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._nethack_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._nethack_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._nethack_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._nethack_load_replay(pointer, length);
    });
    ...
    postMessage({
      type: 'loaded',
      mismatchTick: Module._nethack_mismatch_tick()
    });
  } catch (error) {
    reportFailure(error);
  }
...
function applyInputNow() {
  ...
  try {
    if (Module._nethack_frame() < 0) throw new Error(runtimeError());
    ingestPacket();
    postMessage({
      type: 'inputApplied',
      mismatchTick: Module._nethack_mismatch_tick()
...
importScripts('./wire_constants.js', './broadcast_core.js', './nethack_replay.js');
