...
# use-after-free fixed in nethack_replay.nim (emscripten_exit_with_live_runtime)
...
# nethack_error_len(). Aborting keeps linear memory intact, and the page reads
# nethack_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "nethack_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_nethack_load_replay,_nethack_frame,_nethack_input,_nethack_packet_ptr,_nethack_packet_len,_nethack_mismatch_tick,_nethack_error_ptr,_nethack_error_len,_nethack_stage_ptr,_nethack_stage_len
  """).replace("\n", " ")
)
